...
static const uint16_t ADC_MAX_COUNTS = 3450U;

static const int16_t F_MIN = -40;
static const int16_t F_MAX = 140;

static const int32_t FREEZING = 32; // this has to be signed!
static const int32_t FIVE = 5; // in order to avoid later conversion
static const int32_t FIFTY = 50; // in order to avoid later conversion
static const int32_t NINE = 9; // which would happen in the function
...
static int16_t fahrenheitToCelsius(int16_t fahrenheit) {
    int32_t fahrenheit32 = fahrenheit;
    return (fahrenheit32 - FREEZING) * FIVE / NINE;
}
// I know this decimals handling is a little different
static int16_t fahrenheitToCelsius_decimal(int16_t fahrenheit) {
    int32_t fahrenheit32 = fahrenheit;
    const int32_t celsius_tenfold = (fahrenheit32 - FREEZING) * FIFTY / NINE;
    return (celsius_tenfold < 0) ? (int16_t)(-celsius_tenfold) % 10 : (int16_t)celsius_tenfold % 10;
}
...
void loop(void) {
    ...
    // const int8_t fahrenheit = mapToFahrenheit(counts);
    const uint16_t counts = readPotentiometer();
    // this causes overflow and flips to negative at 127
    // and the  celsius values get distorted too
    ...
    const int16_t celsius_decimal = fahrenheitToCelsius_decimal(fahrenheit);

    Serial.printf("F = %+4d   C = %+4d  (in decimals: %d.%d)\n", (int)fahrenheit, (int)celsius, (int)celsius, (int)celsius_decimal);
}
